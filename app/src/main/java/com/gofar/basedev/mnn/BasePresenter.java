...
public abstract class BasePresenter<M extends BaseModel, V extends BaseView, T> {
    ...
    protected abstract void onHandlerLoading();

    protected abstract void onHandlerResult(T t);

    protected abstract void onHandlerError(Throwable e);

    protected void cacheData(boolean needCache, T t) {
        if (!needCache) {
            return;
        }
